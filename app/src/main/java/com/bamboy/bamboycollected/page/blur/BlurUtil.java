...
public class BlurUtil {
    ...
    public void clickBlurImg(ImageView iv_head_portrait) {
        // 将图片进行高斯模糊，
        // 最后一个参数是模糊等级，值为 0~25
        mActivity.util.bitmap.blurImageView(mActivity, iv_head_portrait, 5);
    }
    ...
    public void clickPopupWindow(RelativeLayout rl_popup_window, ImageView iv_popup_window_back) {
        // 获取截图的Bitmap
        Bitmap bitmap = mActivity.util.ui.getDrawing(mActivity);

        if (mActivity.util.info.phoneSDK >= Build.VERSION_CODES.KITKAT && bitmap != null) {
            // 将截屏Bitma放入ImageView
            iv_popup_window_back.setImageBitmap(bitmap);
            // 将ImageView进行高斯模糊【25是最高模糊程度】【最后一个参数是蒙上一层颜色，此参数可不填】
            // 如果需要更高的模糊程度，可以将此行代码写两遍
            mActivity.util.bitmap.blurImageView(mActivity, iv_popup_window_back, 25, mActivity.getResources().getColor(R.color.colorWhite_t8));
        } else {
        ...
        // 打开弹窗
        mActivity.util.anim.showPopupWindow(rl_popup_window, iv_popup_window_back);
    }
    ...
    public void clickClosePopupWindow(RelativeLayout rl_popup_window, ImageView iv_popup_window_back) {
        mActivity.util.anim.hidePopupWindow(rl_popup_window, iv_popup_window_back);
    }
}
